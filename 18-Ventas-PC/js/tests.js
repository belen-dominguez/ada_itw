...
describe("TP 1 - Ventas de PC", () => {
    ...
    expect(precioMaquina(componentes)).to.be.eql(320);
     ...
     expect(cantidadVentasComponente(componente)).to.be.eql(3);
      ...
      it("ventasVendedora(nombre): Obtener las ventas totales realizadas por una vendedora sin límite de fecha. Ej 05", () => {
        const nombreVendedora = "Grace"
        expect(ventasBusqueda(nombreVendedora)).to.be.eql(900);
    });
    ...
    it("ventasSucursal(sucursal): obtiene las ventas totales realizadas por una sucursal sin límite de fecha -Ej08 (PARTE 2)", () => {
        ...
        expect(ventasBusqueda(sucursal)).to.be.eql(990);
    });
...
/*---------------------------*/
/*
    it("sucursalDelMes(mes, anio): pasa dos parámetros numéricos, (mes, anio) y devuelve el nombre de la sucursal que más vendió en plata en el mes. El importe de una venta es el que indica la función `precioMaquina`. -Ej10", () => {
